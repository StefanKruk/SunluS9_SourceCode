#line 1 "/mnt/hgfs/SVN/Marlin-2.0.x/Marlin/src/module/motion.cpp"
/**
...
void line_to_current_position(const_feedRate_t fr_mm_s/*=feedrate_mm_s*/) {
  ...
  void unscaled_e_move(const_float_t length, const_feedRate_t fr_mm_s) {
    //TERN_(HAS_FILAMENT_SENSOR, runout.reset());//DAVID 20211018
...
void do_z_clearance(const_float_t zclear, const bool lower_allowed/*=false*/) {
  ...
  /*if(zclear>100)
	...
	delay(2000);
	do_blocking_move_to_z(150, 15*60);
	
//    do_z_raise(_MAX(150, Z_CLEARANCE_DEPLOY_PROBE));
	delay(2000);
	do{ MYSERIAL1.print(__FILE__); MYSERIAL1.println(__LINE__); }while(0);
	// ok 	
  	}*/
...
void prepare_line_to_destination() {
  apply_motion_limits(destination);
    ...
    if (!DEBUGGING(DRYRUN) && destination.e != current_position.e) {
       planner.plan_ignore_e = false;
        ...
        planner.plan_ignore_e = thermalManager.tooColdToExtrude(active_extruder);
        if (planner.plan_ignore_e) SERIAL_ECHO_MSG(STR_ERR_COLD_EXTRUDE_STOP);
      #endif
  ...
  feedRate_t get_homing_bump_feedrate(const AxisEnum axis) {
    ...
    static const uint8_t homing_bump_divisor[] PROGMEM = HOMING_BUMP_DIVISOR;
  	...
  	ScreenHandler.Address_Beyond_Fun(&homing_bump_divisor[axis],46);
