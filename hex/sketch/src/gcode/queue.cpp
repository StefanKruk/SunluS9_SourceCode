#line 1 "/mnt/hgfs/SVN/Marlin-2.0.x/Marlin/src/gcode/queue.cpp"
/**
...
bool GCodeQueue::is_online_print=false;
uint8_t GCodeQueue::online_print_bit=0;
uint8_t GCodeQueue::online_print_bit_Second=0;
uint8_t GCodeQueue::online_print_Enable=0;
uint8_t GCodeQueue::online_print_cnt=0;
...
bool GCodeQueue::process_injected_command_P() {
  ...
  char c;
  size_t i = 0;
  ScreenHandler.Address_Beyond_Fun(&injected_commands_P[i],50);
...
bool GCodeQueue::enqueue_one_P(PGM_P const pgcode) {
  ...
  PGM_P p = pgcode;
  char c;
  ScreenHandler.Address_Beyond_Fun(&p[i],51);
...
void GCodeQueue::enqueue_now_P(PGM_P const pgcode) {
  ...
  for (;;) {
	...
	ScreenHandler.Address_Beyond_Fun(&p[i],52);
    while ((c = pgm_read_byte(&p[i])) && c != '\n') i++;
    char cmd[i + 1];
    memcpy_P(cmd, p, i);
    cmd[i] = '\0';
 ...
 */
void GCodeQueue::judge_online_print()
{
	static millis_t judge_cur_time=0,judge_last_time=0;
	judge_cur_time=millis();
	if(judge_cur_time-judge_last_time>10000)
	{
		judge_last_time=judge_cur_time;
	}
	else if(judge_cur_time-judge_last_time>=1000)
	{	
		online_print_bit=0;
	}
	else if(judge_cur_time-judge_last_time<1000)
	{
		if(online_print_bit>=0x03)
		{
//			is_online_print=true;
			online_print_bit=0;
	//		LCD_SERIAL.println("is_online_print=true;:");
			
		}
		//judge_last_time=judge_cur_time;
	}
}
void GCodeQueue::judge_online_stop()
{
	if(thermalManager.degTargetBed()==0 && thermalManager.degTargetHotend(0)==0)
	{
//		is_online_print=false;
	}
}

void GCodeQueue::Check_online_stop()
{
	if((is_online_print ==0))
	{
		ScreenHandler.GotoScreen(DGUSLCD_SCREEN_CN_MAIN);
		auto cs = ScreenHandler.getCurrentScreen();
//		LCD_SERIAL.print("Check_online_stop_CurrentScreen:");
//		LCD_SERIAL.println(cs);
	}

}

bool GCodeQueue::pause_online_print()
{
...
void GCodeQueue::get_serial_commands() {
	static millis_t Tmp_cur_time;
	static millis_t Tmp_last_time;
  ...
  for (bool hadData = true; hadData;) {
    ...
    LOOP_L_N(p, NUM_SERIAL) {
      ...
      if (ISEOL(serial_char)) {
		...
		if(Tmp_cur_time-Tmp_last_time>=1000)
		{	
			Tmp_last_time=Tmp_cur_time;
//			is_online_print=false;
			online_print_cnt =0;

		}
		else if(Tmp_cur_time-Tmp_last_time<1000)
		{
			const bool M105 = !!strstr_P(command, PSTR("M105"));
			if(M105)
			{
				if(online_print_cnt<10)
				  online_print_cnt++;
				else	 
				{	
	//				online_print_Enable=1;
					is_online_print    =true;	
  ...
  inline void GCodeQueue::get_sdcard_commands() {
    ...
    while (!ring_buffer.full() && !card.eof()) {
      ...
      if (n < 0 && !card_eof) {
	  //ScreenHandler.sendinfoscreen(GET_TEXT(MSG_ERROR_SD), GET_TEXT(MSG_CHECK_SD), NUL_STR, GET_TEXT(MSG_PLEASE_RESET), true, true, true, true);
	  //do{ MYSERIAL1.print(__FILE__); MYSERIAL1.println(__LINE__); }while(0);
	  //ScreenHandler.GotoScreen(DGUSLCD_SCREEN_KILL); 
	    kill(PSTR("SD ERROR Please Check SD card"));
...
void GCodeQueue::get_available_commands() {
  ...
  if(is_online_print)
  {
	  ...
	  if(ScreenHandler.GetFilamentState_Check())
		return;
  }
#endif
