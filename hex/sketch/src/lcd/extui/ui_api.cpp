#line 1 "/mnt/hgfs/SVN/Marlin-2.0.x/Marlin/src/lcd/extui/ui_api.cpp"
/**
...
namespace ExtUI {
  ...
  bool isHeaterIdle(const heater_t heater) {
      ...
      switch (heater) {
        ...
        #endif
        #if ENABLED(HAS_HEATED_DRYBOX)
          case DRYBOX: return thermalManager.heater_idle[thermalManager.IDLE_INDEX_DRYBOX].timed_out;
        #endif
        #if ENABLED(HAS_HEATED_CHAMBER)
  ...
  void setAxisMaxAcceleration_mm_s2(const_float_t value, const extruder_t extruder) {
    ...
    bool getTestFilamentRunoutState()               { return runout.test_filament_ran_out; }
    void setTestFilamentRunoutState(const bool value)   { runout.test_filament_ran_out = value; }
    void ResetFilamentRunout()   					{ runout.reset(); }
    void setFilamentRunout_enabled(int8_t en)		{ runout.setFilamentRunout_enabled(en);}
    int8_t getFilamentRunout_enabled()		{ return runout.enabled;}
    ...
    void smartAdjustAxis_steps(const int16_t steps, const axis_t axis, bool linked_nozzles) {
      ...
      if (!babystepAxis_steps(steps, axis)) return;
        ...
        if (axis == Z && TERN1(HAS_MULTI_EXTRUDER, (linked_nozzles || active_extruder == 0)))
          ...
          probe.offset.z += mm;  //2021-10-08 zb  mm (-mmm)
  ...
  void setZOffset_mm(const_float_t value) {
    ...
    void setProbeOffset_Z_mm(const_float_t val) { probe.offset.pos[Z_AXIS] += val; }
  #endif
  ...
  void enqueue_one_now(char * const gcode)  { queue.enqueue_one_now(gcode); }
  bool enqueue_one_P(PGM_P const gcode)  { return queue.enqueue_one_P(gcode); }
  ...
  bool awaitingUserConfirm() { return wait_for_user|| getHostKeepaliveIsPaused(); }//zb_ok
...
void MarlinUI::init() { ExtUI::onStartup(ui.language); }
