#line 1 "/mnt/hgfs/SVN/Marlin-2.0.x/Marlin/src/lcd/extui/dgus/DGUSDisplay.cpp"
/**
...
void DGUSDisplay::WriteVariable(uint16_t adr, const void *values, uint8_t valueslen, bool isstr) {
  ...
  while (valueslen--) {
	...
	//if(adr==VP_GetCFG) do{ MYSERIAL1.println(x); }while(0);
	
  }
}
void DGUSDisplay::ReadVariable(uint16_t adr, const void *values, uint8_t valueslen, bool isstr) {
  const char* myvalues = static_cast<const char*>(values);
  bool strend = !myvalues;

  WriteHeader(adr, DGUS_CMD_READVAR, valueslen);
  while (valueslen--) {
    char x;
    if (!strend) x = *myvalues++;
    if ((isstr && !x) || strend) {
...
void DGUSDisplay::WriteVariablePGM(uint16_t adr, const void *values, uint8_t valueslen, bool isstr) {
  ...
  while (valueslen--) {
    char x;
	ScreenHandler.Address_Beyond_Fun(myvalues,5);
  ...
  while (LCD_SERIAL.available()) {
    switch (rx_datagram_state) {
      ...
      case DGUS_WAIT_TELEGRAM: // wait for complete datagram to arrive.
        ...
        |      tmp[0  1  2  3  4 ... ]
        | Example 5A A5 06 83 20 01 01 78 01 â€¦â€?        |          / /  |  |   \ /   |  \     \
...
void DGUSDisplay::WritePGM(const char str[], uint8_t len) {
  ...
  while (len--){
  	ScreenHandler.Address_Beyond_Fun(str,6);
  	LCD_SERIAL.write(pgm_read_byte(str++));
  	}
}

void DGUSDisplay::loop() {
  // protect against recursionâ€?ProcessRx() may indirectly call idle() when injecting gcode commands.
  if (!no_reentrance) {
    no_reentrance = true;
    ProcessRx();
    no_reentrance = false;
