#line 1 "/mnt/hgfs/SVN/Marlin-2.0.x/Marlin/src/feature/runout.h"
/**
...
class TFilamentMonitor : public FilamentMonitorBase {
  ...
  public:
    ...
    // 第一次出发断料之后，恢复打印就不再进入下面程序
    // 解决思路，点了恢复打印之后，先出料 FILAMENT_RUNOUT_DISTANCE_MM ，判断是否有料，如果有，就继续打印
    // 下面的判断语句，要再加一个判断条件
...
/********************************* RESPONSE TYPE *********************************/
  ...
  class RunoutResponseDelayed {
    ...
    public:
      ...
      static inline void block_completed(const block_t * const b) {//需要在这里获取脉冲数
