#line 1 "/mnt/hgfs/SVN/Marlin-2.0.x/Marlin/src/feature/powerloss.cpp"
/**
...
void PrintJobRecovery::load() {
 ...
 // do{ MYSERIAL1.print("info.sd_filename"); MYSERIAL1.println(info.sd_filename); }while(0);
    close();
  }
  debug(PSTR("Load"));
}
...
void PrintJobRecovery::write() {
  ...
  open(false);
  file.seekSet(0);
  memset(info.last_filename,'\0',sizeof(info.last_filename));
//  snprintf_P(info.last_filename, MAXPATHNAMELENGTH, PSTR("%s"), filelist.filename());
  snprintf_P(info.last_filename, MAXPATHNAMELENGTH, PSTR("%s"), card.longest_filename());
  // do{ MYSERIAL1.print(__FILE__); MYSERIAL1.println(__LINE__); }while(0);
//  do{ MYSERIAL1.print("last_filename:"); MYSERIAL1.println(info.last_filename); }while(0);
...
void PrintJobRecovery::resume() {
  ...
  #if HAS_HOTEND
    HOTEND_LOOP() {
      const celsius_t et = info.target_temperature[e];//_MAX(info.target_temperature[e], 180);
      if (et) {
        #if HAS_MULTI_HOTEND
